...
public abstract class FlashNode : FlashAPI
{
    ...
    protected RemoteAPI!TestAPI agora_node;  // random agora node
    ...
    public override Result!Signature requestSettleSig (in Hash chan_id, in uint seq_id)
    {
        ...
        return Result!Signature(ErrorCode.WrongChannelID, "Channel ID not found");
    }
        ...
        in uint seq_id, in BalanceRequest balance_req)
    {
        ...
        if (!channel.isOpen())
            return Result!PublicNonce(ErrorCode.ChannelNotFunded,
