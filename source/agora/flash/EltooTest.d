...
unittest
{
    ...
    Transaction update_2 = {
        seq_id: seq_id_1,  // may attach to a previous update
        type: TxType.Payment,
        inputs: [Input(funding_tx, 0 /* index */, 0 /* no unlock age */)],
        outputs: [
            Output(FundingAmount,
    ...
    // validate that `update_2` can attach to funding tx
    test!("==")(engine.execute(
        funding_tx.outputs[0].lock, update_2_unlock, update_2,
            update_2.inputs[0]),
        null);

    // validate that `update_2` can attach to `update_1`
    test!("==")(engine.execute(
