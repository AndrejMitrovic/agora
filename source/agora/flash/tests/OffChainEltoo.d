...
module agora.flash.tests.OffChainEltoo;
...
public Unlock createUnlockSettle (Signature sig, in uint sequence)
    pure nothrow @safe
{
...
public Unlock createUnlockUpdate (Signature sig, in uint sequence)
    pure nothrow @safe
{
...
unittest
{
    ...
    const seq_id_0 = uint(0);
    const seq_id_1 = uint(1);
    const seq_id_2 = uint(2);
    const seq_id_3 = uint(3);
    const seq_id_4 = uint(4);

    // always signing the transaction with the lone Input blanked
    const Input_0 = 0;
    ...
    const kim_funding_sig = sign(kim_funding_kp.v, kim_funding_kp.V,
        kim_nonce.V, kim_nonce.v, funding_tx);
    assert(verify(kim_funding_kp.V, kim_funding_sig, funding_tx)); // ok
    ...
    const challenge_settle_0 = getSequenceChallenge(settle_0, seq_id_1, Input_0);
    ...
    const Unlock settle_0_unlock = createUnlockSettle(settle_0_multi_sig,
        seq_id_1);
    ...
    const bob_trigger_sig = sign(bob_update_kp.v, UX, RX, bob_nonce.v,
        trigger_tx);
    assert(!verify(UX, bob_trigger_sig, trigger_tx));  // not valid yet
    ...
    assert(verify(UX, trigger_multi_sig, trigger_tx));
    ...
    // this signals the payment channel has been created.
    ...
    const challenge_update_1 = getSequenceChallenge(update_1, seq_id_1, Input_0);
    ...
    const challenge_settle_1 = getSequenceChallenge(settle_1, seq_id_2, Input_0);
    ...
    // the unlock settlement script is created
    const Unlock settle_1_unlock = createUnlockSettle(settle_1_multi_sig,
        seq_id_2);
    ...
    const Unlock update_1_unlock = createUnlockUpdate(update_1_multi_sig,
        seq_id_1);
    ...
    const challenge_update_2 = getSequenceChallenge(update_2, seq_id_2, Input_0);
    ...
    const challenge_settle_2 = getSequenceChallenge(settle_2, seq_id_3, Input_0);
    ...
    // the unlock settlement script is created
    const Unlock settle_2_unlock = createUnlockSettle(settle_2_multi_sig,
        seq_id_3);
    ...
    const Unlock update_2_unlock = createUnlockUpdate(update_2_multi_sig,
        seq_id_2);
    ...
    test!("==")(engine.execute(
        update_2.outputs[0].lock, update_1_unlock, update_1,
            update_1.inputs[0]),
        "VERIFY_SEQ_SIG sequence is not equal to or greater than min_sequence");
    ...
    // now we publish the trigger tx
    version (all)
    {
        // we only publish the trigger once we want to start closing the channel
        utxo_set.updateUTXOCache(trigger_tx, Height(2));

        // The UTXO spent by the trigger tx is now gone
        assert(!utxo_set.peekUTXO(trigger_tx.inputs[0].utxo, _utxo));
        // settlement can refer to the new trigger UTXO, however we will not
        // publish the settlement yet (it's encumbered by a time-lock anyway)
