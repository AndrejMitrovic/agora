...
unittest
{
    ...
    test!"=="(validateScript(ScriptType.Lock, [2, 255, 255], StackMaxItemSize,
        result), null);
    ubyte[64] payload_64;
    test!"=="(validateScript(ScriptType.Lock, [ubyte(64)] ~ payload_64[0 .. 63],
        StackMaxItemSize, result),
        "PUSH_BYTES_* opcode exceeds total script size");
    // 64-byte data payload
    test!"=="(validateScript(ScriptType.Lock, [ubyte(64)] ~ payload_64,
        StackMaxItemSize, result), null);
...
unittest
{
    import std.array;
    import std.range;
    test!("==")(ubyte(42).repeat(64).array.toPushOpcode(),
        [64] ~ 42.repeat(64).array);
