...
public class Engine
{
    ...
    ***************************************************************************/
        ...
        in Transaction tx, in Input input) nothrow @safe
    {
        ...
        while (!bytes.empty())
        {
            ...
            case OP.PUSH_DATA_1:
                if (auto reason = this.handlePushData!(OP.PUSH_DATA_1)(
                    stack, bytes))
                    return reason;
                break;

            case OP.PUSH_DATA_2:
                if (auto reason = this.handlePushData!(OP.PUSH_DATA_2)(
                    stack, bytes))
                    return reason;
                break;
    ...
    ***************************************************************************/
    ...
    /***************************************************************************
        ...
        tries to push the payload onto the stack, and if successfull it
        advances the `opcodes` array to the next opcode.
    ...
    ***************************************************************************/
    ...
    private string handlePushData (OP op)(ref Stack stack,
        ref const(ubyte)[] opcodes) nothrow @safe /*@nogc*/
    {
        ...
        {
            import std.conv : to;
            static immutable err = "Stack overflow while executing "
                ~ op.to!string;
            return err;
        }
