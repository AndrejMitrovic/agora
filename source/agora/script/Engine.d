...
public class Engine
{
    ...
    ***************************************************************************/
        ...
        nothrow @safe //@nogc  // stack.pop() is not @nogc
    {
        ...
        // top to bottom: <min_seq> <key> [<seq> <sig>]
        if (stack.count() < 4)
        {
            static immutable err1 = op.to!string
        ...
        uint min_sequence;
        if (!readUnsignedInteger!uint(min_seq_bytes, min_sequence))
        {
            static immutable err4 = op.to!string
                ~ " opcode requires 4-byte minimum sequence on the stack";
            return err4;
        }
        ...
        uint sequence;
        if (!readUnsignedInteger!uint(seq_bytes, sequence))
        {
            static immutable err3 = op.to!string
                ~ " opcode requires 4-byte sequence on the stack";
            return err3;
        }
...
public Hash getSequenceChallenge (in Transaction tx, in uint sequence,
