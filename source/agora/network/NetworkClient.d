    ...
    ***************************************************************************/
        ...
        API api, Duration retry, size_t max_retries)
    {
        ...
        this.exception = new Exception(
            format("Request failure to %s after %s attempts", address,
                max_retries));
        this.taskman.setTimer(0.seconds, &this.gossipScheduler);
    }

    /// For gossiping we don't want to block the calling fiber, so we use
    /// use request queueing and a separate fiber to handle all the outgoing requests.
    private void gossipScheduler ()
    {
        while (1)
        {
            ...
            this.taskman.wait(10.msecs);  // yield
        }
    }
    ...
    private void handleGossip (GossipEvent event) nothrow
    {
        ...
        case Tx:
            ...
            const tx_hash = event.tx.hashFull();
