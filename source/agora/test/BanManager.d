...
unittest
{
        ...
        [node_1, node_2].enumerate.each!((idx, node) =>
            retryFor(node.getBlockHeight() == 1 + (block_idx + 1),
                ...
                format("Block %s Node %s has block height %s. Expected: %s",
                    block_idx + 1, idx, node.getBlockHeight().to!string, 1 + (block_idx + 1))));
    ...
    // sanity check: block height should not updated, node 3 is not a validator and cannot make blocks
    node_3.clearFilter();
    ...
    nodes.enumerate.each!((idx, node) =>
        retryFor(node.getBlockHeight() == 6,
            ...
            format("Node %s has block height %s. Expected: %s",
                idx, node.getBlockHeight().to!string, 6)));
}
