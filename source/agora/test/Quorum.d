...
unittest
{
    // one way to test it: a validator is no longer enrolled, other nodes
    // don't care about communicating with it. we could force the node
    // to disagree with proposals. if the node is still in the enrolled set,
    // it will stop progress of the blockchain. otherwise it should properly
    // continue because this node is no longer part of the validator set.
    ...
    // verify there are only 2 validators now
