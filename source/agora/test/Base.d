    ...
    in TestConf test_conf)
{
    ...
    auto gen_block = makeGenesisBlock(node_configs);
    ...
    immutable(Block)[] blocks = generateBlocks(gen_block, node_configs,
...
/*******************************************************************************
    ...
    Generate a genesis block given the set of node configurations.

    For each Validator in the node configs, a Freeze TX and an Enrollment
    ...
    Params:
        node_configs = the list of node configurations
...
private immutable(Block) makeGenesisBlock (in NodeConfig[] node_configs)
{
    ...
    foreach (node_conf; node_configs)
    {
        if (node_conf.is_validator)
        {
            Transaction tx =
            {
                type : TxType.Freeze,
                ...
                outputs : [Output(Amount.MinFreezeAmount,
                    node_conf.key_pair.address)]
            };

            txs ~= tx;
            Hash txhash = hashFull(tx);
            Hash utxo = UTXOSet.getHash(txhash, 0);
            scope enr = new EnrollmentManager(":memory:", node_conf.key_pair);
...
private immutable(Block)[] generateBlocks (
    ref immutable Block gen_block, in NodeConfig[] node_configs, size_t count)
{
