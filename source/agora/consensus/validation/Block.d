...
public string isInvalidReason (const ref Block block, Engine engine,
    Height prev_height, in Hash prev_hash, scope UTXOFinder findUTXO,
...
unittest
{
    ...
    assert(!block.isValid(engine, Height(0), Hash.init, null,
...
unittest
{
    ...
    const reason = block2.isInvalidReason(engine, block1.header.height,
        hashFull(block1.header), findUTXO, Enrollment.MinValidatorCount);
    assert(reason is null, reason);
...
unittest
{
    ...
    auto block2 = makeNewBlock(block1, txs_2);
    assert(block2.isValid(engine, block1.header.height, hashFull(block1.header),
        findUTXO, Enrollment.MinValidatorCount));
    foreach (ref tx; txs_2)
        utxo_set.put(tx);
    ...
    // in the new block is 0, the block is considered invalid.
    {
        ...
        assert(!block3.isValid(engine, block2.header.height,
            hashFull(block2.header), findUTXO, 0));
    }
    ...
    // then new block does not need to contain new enrollments to be considered valid
    {
        ...
        findUTXO = utxo_set.getUTXOFinder();
        assert(block3.isValid(engine, block2.header.height,
            hashFull(block2.header), findUTXO, 1));
    }
}
