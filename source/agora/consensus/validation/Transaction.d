...
version (unittest)
public bool isValid (in Transaction tx, Engine engine, scope UTXOFinder findUTXO, in Height height)
    @safe nothrow
{
    return isInvalidReason(tx, engine, findUTXO, height) is null;
}
...
unittest
{
    ...
    assert(secondTx.isValid(engine, storage.getUTXOFinder(), Height(0)),
...
unittest
{
    ...
    assert(!tx_2.isValid(engine, storage.getUTXOFinder(), Height(0)));
    ...
    assert(!tx_3.isValid(engine, storage.getUTXOFinder(), Height(0)));
}
...
unittest
{
    ...
    assert(tx1.isValid(engine, storage.getUTXOFinder(), Height(0)),
    ...
    storage.put(tx2);
    assert(!tx2.isValid(engine, storage.getUTXOFinder(), Height(0)));
}
...
unittest
{
    ...
    // Expected Status : frozen
    {
        ...
        // Second Transaction is VALID.
        assert(secondTx.isValid(engine, storage.getUTXOFinder(), block_height));
    ...
    // Expected Status : melting
    {
        ...
        assert(thirdTx.isValid(engine, storage.getUTXOFinder(), block_height));
    ...
    // Expected Status : melting
    {
        ...
        assert(!fourthTx.isValid(engine, storage.getUTXOFinder(), block_height));
    }
    ...
    // Expected Status : melted
    {
        ...
        // Third Transaction is VALID.
        assert(fifthTx.isValid(engine, storage.getUTXOFinder(), block_height));
...
unittest
{
    ...
    assert(canFind(toLower(oneTx.isInvalidReason(engine, storage.getUTXOFinder(), Height(0))), "no input"),
    ...
    assert(canFind(toLower(secondTx.isInvalidReason(engine, storage.getUTXOFinder(), Height(0))), "no output"),
...
unittest
{
    ...
    // test for transaction having combined inputs
    assert(!thirdTx.isValid(engine, storage.getUTXOFinder(), Height(0)),
...
unittest
{
    ...
    // test for unknown transaction type
    assert(!firstTx.isValid(engine, null, Height(0)),
...
unittest
{
    ...
    assert(!thirdTx.isValid(engine, &storage.peekUTXO, Height(0)),
    ...
    assert(!fourthTx.isValid(engine, &storage.peekUTXO, Height(0)),
...
unittest
{
    ...
    test!"=="(tx.isInvalidReason(engine, storage.getUTXOFinder(), Height(0)), null);
    test!"=="(tx.isInvalidReason(engine, storage.getUTXOFinder(), Height(1024)), null);

    tx.height_lock = Height(10);
    tx.inputs[0].unlock = signUnlock(kp, tx);
    test!"=="(tx.isInvalidReason(engine, storage.getUTXOFinder(), Height(0)),
        "Transaction: Not unlocked for this height");
    test!"=="(tx.isInvalidReason(engine, storage.getUTXOFinder(), Height(9)),
        "Transaction: Not unlocked for this height");
    test!"=="(tx.isInvalidReason(engine, storage.getUTXOFinder(), Height(10)),
        null);
    test!"=="(tx.isInvalidReason(engine, storage.getUTXOFinder(), Height(1024)),
        null);
}
