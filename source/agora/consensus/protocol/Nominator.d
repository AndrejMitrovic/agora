...
public extern (C++) class Nominator : SCPDriver
{
    ...
    private uint[PublicKey] pubkey_indices;
...
extern(D):
    ...
    ***************************************************************************/
    ...
    /***************************************************************************
        ...
        Update the public key indices to be able to set the
        bitfield in the block header when collecting signatures.

        Params:
            keys = the public keys of the active validators
            height = the height at which this validator set is active

    ***************************************************************************/

    public void updateValidatorIndices (PublicKey[] keys, Height height)
        @safe nothrow
    {
        ...
        () @trusted { this.pubkey_indices.clear(); }();
        foreach (idx, key; keys_arr)
            () @trusted { this.pubkey_indices[key] = cast(uint)idx; }();
    }
    ...
    ***************************************************************************/
    ...
    private void restoreSCPState (Ledger ledger)
    {
        ...
        auto pub_key = NodeID(uint256(PublicKey(this.key_pair.V[])));
    ...
    ***************************************************************************/
        ...
        ref const(Value) value) nothrow
    {
        ...
        try
        {
            ...
            Scalar multi_sig;
            auto signed_mask = BitField!uint(block_sigs.length);
            foreach (K, sig; *block_sigs)
            {
                multi_sig = (multi_sig == Scalar.init)
                    ? sig : multi_sig + sig;  // adding to Scalar.init is wrong

                signed_mask[this.pubkey_indices[PublicKey(K[])]] = true;
            }
            ...
            proposed_block.header.validators = signed_mask;
