...
/// 8 transactions - hence the magic number `8` appearing everywhere.
/// This used to be a constant but this limitation has been lifted.
version (unittest)
{
    ...
    {
        ConsensusData data;
        ledger.prepareNominatingSet(data, 8);
        assert(data.tx_set.length == 8);
...
unittest
{
    ...
    void genBlockTransactions (size_t count)
    {
        ...
        if (!last_txs.length)
        {
            last_txs = genesisSpendable().take(8).enumerate()
    ...
    const Transaction default_tx)
{
    ...
    assert(in_key_pair.length == 8);
    assert(out_key_pair.length == 8);
    ...
    foreach (idx; 0 .. TxCount)
    {
        ...
        Transaction tx =
        {
            tx_type,
            [input],
            [Output(AmountPerTx, out_key_pair[idx % 8].address)]  // send to the same address
        };

        auto signature = in_key_pair[idx % 8].secret.sign(hashFull(tx)[]);
...
unittest
{
    ...
    txs.enumerate()
        .map!(en => TxBuilder(en.value)
              .refund(WK.Keys[8 + en.index].address)
              .sign(TxType.Freeze))
    ...
    splited_txex = () {
        ...
        foreach (idx; 0 .. 8)
        {
            txs ~= TxBuilder(params.Genesis.txs[1], idx)
                .split(splited_keys[idx].address.repeat(8))
                .sign();
        }
        return txs;
    }();
