...
xvector!SCPQuorumSet nullvec;
shared_ptr!SCPQuorumSet nullquorum;

size_t first (size_t[2] pair) { return pair[0]; }
size_t second (size_t[2] pair) { return pair[1]; }

auto makeSharedQuorumSet (T1, T2, T3)(T1 threshold, T2 nodes, T3 quorums)
{
    QuorumConfig qc =
    {
        threshold : threshold,
                 ...
                 size_t ownThreshPct = 67, size_t innerThreshPct = 51)
{
    ...
    for (size_t i = 0; i < orgs.length; ++i)
    {
        const org = &orgs[i];
        SCPQuorumSet qq;
        auto qs = makeSharedSCPQuorumSet(qq);

        foreach (x; *org)
            qs.validators.push_back(x);
        ...
        for (size_t j = 0; j < orgs.length; ++j)
        {
            ...
            if (shouldDepend(i, j))
            {
                ...
                foreach (NodeID org2; otherOrg)
                    other_orgs.push_back(org2);
